...
This layout seems clean since it separates the development part (Cargo files, src directory) and the web deployment part (www). It has however on serious inconvenient, which only discovered recently:
...
>
> Well then, let us just drop the `module=`part and modify lib.rs as follows:
```rust
#[wasm_bindgen]
extern "C" {
    fn minusesc_plusshort();
